...
__global__ void depth_image_kernel(
    ...
    const float* __restrict__ in_index,
    ...
    unsigned int size, unsigned int width, unsigned int height) {
        ...
        if(index >= 0 && index < size) {
            ...
            out_idx[uv_index] = in_index[index]; // label the point cloud index on a image
        }
...
__global__ void visibility_kernel2(
    ...
    const scalar_t* __restrict__ in_index,
    scalar_t* __restrict__  out,
    scalar_t* __restrict__  out_index,
    ...
    out_index[index] = in_index[index];
    bool debug=false;
...
template <typename scalar_t>
__global__ void image_warp_index_kernal(
    const scalar_t* __restrict__ in,
    const int* __restrict__ flo,
    scalar_t* __restrict__ warp,
    scalar_t* __restrict__ out,
    ...
    //unsigned int index = y*width + x;
    unsigned int index = x*width + y;
    //int flow_x = flo[2*index];
    //int flow_y = flo[2*index+1];
    int flow_y = flo[index];
    int flow_x = flo[width*height+index];
    int out_x = x+flow_x;
    int out_y = y+flow_y;

    if ((flow_x != 0 || flow_y != 0) && out_x >= 0 && out_x < height && out_y >= 0 && out_y < width) {
        unsigned int out_index = out_x*width + out_y;
        if (warp[out_index] > in[index]){
                warp[out_index] = in[index];
                out[index] = out_x;
                out[index+width*height] = out_y;
        }
        else {
                warp[out_index] = warp[out_index];
        }
        //out[index] = out_x;
        //out[index+width*height] = out_y;
    }
...
template <typename scalar_t>
__global__ void image_warp_kernal(
    const scalar_t* __restrict__ in,
    const int* __restrict__ flo,
    scalar_t* __restrict__ out,
    unsigned int width, unsigned int height){
    ...
    if ((flow_x != 0 || flow_y != 0) && out_x >= 0 && out_x < height && out_y >= 0 && out_y < width) {
        unsigned int out_index = out_x*width + out_y;
        if (out[out_index] > in[index]){
                out[out_index] = in[index];
        }
        else {
                out[out_index] = out[out_index];
        }
        //out[out_index] = in[index];
    }
}
...
std::vector<at::Tensor> depth_image_cuda(at::Tensor input_uv, at::Tensor input_depth, at::Tensor input_index, at::Tensor output, at::Tensor output_index, unsigned int size, unsigned int width,unsigned int height) {
    ...
    depth_image_kernel<<<blocks, threads>>>(input_uv.data<int>(), input_depth.data<float>(), input_index.data<float>(), output.data<float>(), output_index.data<float>(), size, width, height);
...
at::Tensor visibility_filter_cuda(at::Tensor input, at::Tensor output, unsigned int width,unsigned int height, unsigned int threshold) {
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "visibility_filter", ([&] {
        visibility_kernel<scalar_t><<<blocks, threads>>>(input.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(), width, height, threshold);}));
    //visibility_kernel<float><<<blocks, threads>>>(input.data<float>(), output.data<float>(), width, height, threshold);
    return output;
}
...
std::vector<at::Tensor> visibility_filter_cuda2(at::Tensor input_depth, at::Tensor intrinsic, at::Tensor in_index, at::Tensor output, at::Tensor out_index, unsigned int width,unsigned int height, float threshold, unsigned int radius) {
    ...
    AT_DISPATCH_FLOATING_TYPES(input_depth.scalar_type(), "visibility_filter2", ([&] {
        visibility_kernel2<scalar_t><<<blocks, threads>>>(input_depth.data_ptr<scalar_t>(), intrinsic.data_ptr<scalar_t>(), in_index.data_ptr<scalar_t>(),
                                                          output.data_ptr<scalar_t>(),out_index.data_ptr<scalar_t>(), width, height, threshold, radius);}));
    //visibility_kernel<float><<<blocks, threads>>>(input.data<float>(), output.data<float>(), width, height, threshold);
...
at::Tensor downsample_flow_cuda(at::Tensor input_uv, at::Tensor output, unsigned int width_out ,unsigned int height_out, unsigned int kernel) {
    ...
    AT_DISPATCH_FLOATING_TYPES(input_uv.scalar_type(), "downsample_flow", ([&] {
        downsample_flow_kernel<scalar_t><<<blocks, threads>>>(input_uv.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(), 
...
at::Tensor image_warp_index_cuda(at::Tensor input, at::Tensor flow, at::Tensor warp_depth, at::Tensor output, unsigned int width, unsigned int height)
{
    dim3 threads(32, 32);
    dim3 blocks(height/32+1, width/32+1);
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "image_warp_index",([&] {
    image_warp_index_kernal<<<blocks, threads>>>(input.data_ptr<scalar_t>(), flow.data_ptr<int>(), warp_depth.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(), width, height);}));
    return output;
}
...
at::Tensor image_warp_cuda(at::Tensor input, at::Tensor flow, at::Tensor output, unsigned int width, unsigned int height)
{
    dim3 threads(32, 32);
    dim3 blocks(height/32+1, width/32+1);
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "image_warp",([&] {
    image_warp_kernal<<<blocks, threads>>>(input.data_ptr<scalar_t>(), flow.data_ptr<int>(),output.data_ptr<scalar_t>(), width, height);}));
    return output;
}
